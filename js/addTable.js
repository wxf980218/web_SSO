...
$(function () {
    ...
    function ztreeOnclick(event,treeId, treeNode){
        ...
        if(treeNode.level == 2){
            //判断日期(测试日期，2019年12月)
            if(year==treeNode.趋势表["日期"]["年"]&&
              month==treeNode.趋势表["日期"]["月"]){
                //table表填内容
                index = treeNodeInfo.index;  //获取结点在JSON数组中的下表
                $("#11z").val(tempJson[index]["趋势表"]["日期"]["表"]["个人素质"]["1"].直接领导考核);
                $("#11j").val(tempJson[index]["趋势表"]["日期"]["表"]["个人素质"]["1"].分管领导考核);
                $("#11bei").val(tempJson[index]["趋势表"]["日期"]["表"]["个人素质"]["1"].备注);
                $("#12z").val(tempJson[index]["趋势表"]["日期"]["表"]["个人素质"]["2"].直接领导考核);
                $("#12j").val(tempJson[index]["趋势表"]["日期"]["表"]["个人素质"]["2"].分管领导考核);
                $("#12bei").val(tempJson[index]["趋势表"]["日期"]["表"]["个人素质"]["2"].备注);
                $("#13z").val(tempJson[index]["趋势表"]["日期"]["表"]["个人素质"]["3"].直接领导考核);
                $("#13j").val(tempJson[index]["趋势表"]["日期"]["表"]["个人素质"]["3"].分管领导考核);
                $("#13bei").val(tempJson[index]["趋势表"]["日期"]["表"]["个人素质"]["3"].备注);
                $("#14z").val(tempJson[index]["趋势表"]["日期"]["表"]["个人素质"]["4"].直接领导考核);
                $("#14j").val(tempJson[index]["趋势表"]["日期"]["表"]["个人素质"]["4"].分管领导考核);
                $("#14bei").val(tempJson[index]["趋势表"]["日期"]["表"]["个人素质"]["4"].备注);
                $("#15z").val(tempJson[index]["趋势表"]["日期"]["表"]["个人素质"]["合计"].直接领导考核);
                $("#15j").val(tempJson[index]["趋势表"]["日期"]["表"]["个人素质"]["合计"].分管领导考核);
                $("#15bei").val(tempJson[index]["趋势表"]["日期"]["表"]["个人素质"]["合计"].备注);
            }else{
                $('input').val("");//置空
            }
