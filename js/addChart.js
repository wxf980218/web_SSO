...
$(function () {
    ...
    var ztree,ztreeJson,appraisalJson;
    //通过回调函数获取日期（年、月）
    ...
    function draw(year,month,treeNode) {
    ...
    function darwZtree() {
        ...
        $(document).ready(function(){
            ...
            setTimeout(getSelectedInfo(),5000);
        });
    }
    ...
    function getSelectedInfo(){
        treeNodeInfo = JSON.parse(localStorage.getItem("tendencyInfo"));
        year = treeNodeInfo.year;
        month = treeNodeInfo.month;
        $('input.layui-input').attr('placeholder',year+"-"+month);
        ztree = $.fn.zTree.getZTreeObj("treeDemo");
        //设置结点为选中状态
        var treeNode = ztree.getNodeByParam("id",treeNodeInfo.id);
        ztree.selectNode(treeNode);
        draw(year,month,treeNodeInfo);
    }
